...
* **自动备份**：新建礼簿后会自动生成一份加密 BIN 备份；首次可授权项目根目录，后续会自动写入该目录（浏览器不支持目录写入时自动退化为下载）。
...
如果你希望“导出 JSON/BIN”直接写入网站根目录（而不是仅下载到浏览器），可使用本仓库自带的 Node 服务：
...
2. 打开 `http://localhost:8080` 使用系统。
3. 导出备份时，前端会优先调用 `/api/backups/upload`，服务器会把备份写入根目录下的 `backups/` 文件夹。
4. 若接口不可用，系统会自动回退为浏览器下载，不影响原有使用。

> 可通过环境变量 `PORT` 修改端口，`BACKUP_DIR_NAME` 修改备份目录名。
